...
public class ThreadF1Cars extends Thread {
	...
	private void makeLaps(int qtyLaps) {
		for (int x = 0; x < qtyLaps; x++) {
			...
			System.out.println("O Carro #" + this.idCar + " Da Scuderia #" + this.idScuderia + " Fez a" + (x + 1)
					+ "ª Volta  0" + ((lapTime / 1000) / 60) + ":0" + ((lapTime / 1000)-60) + "." + (lapTime % 1000));
		}
	}
